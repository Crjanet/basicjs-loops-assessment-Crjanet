// To run this assignment, right click on index.html in the Visual Studio Code file explorer to the left
// and select "Open with Live Server"

let numbers = [
    469, 755, 244, 245, 758, 450, 302, 20, 712, 71, 456, 21, 398, 339, 882, 848,
    179, 535, 940, 472,
];

document.write("<h1>Array Analyzer</h1>");
let displayNumbers = function() { // This function will display the array to the DOM
    return numbers;
}
document.write("<h3>Now analyzing the array: " + displayNumbers() + "</h3>");

// We want all answers to be rendered using an unordered list
document.write("<ul>");
// That means each answer will be rendered with li
// For example:
//     document.write("<li>First Number: " + numbers[0] + "</li>");
//     document.write("<li>Second Number: " + numbers[1] + "</li>");

// Write your code here 👇

let largestNumber = 0
let smallestNumber = numbers[0]
let sum = 0
for (let index = 0; index < numbers.length; index++) {
    let currentNumber = numbers[index]
    sum += numbers[index]
    if (currentNumber > largestNumber) {
        largestNumber = currentNumber
    }
    if (numbers[index] < smallestNumber) {
        smallestNumber = currentNumber
    }
}

let avg = sum / numbers.length
let evens = numbers.filter(number => {
    return number % 2 === 0
})
let odds = numbers.filter(number => {
    return number % 2 !== 0
})
let divisible = numbers.filter(number =>{
    return number % 8 === 0
})

console.log(largestNumber)
console.log(smallestNumber)
console.log(sum)
console.log(avg)
console.log(evens)
console.log(odds)
console.log(divisible)
document.write("<h3><li>Largest Number: " + largestNumber + "</li></h3>")
document.write("<h3><li>Smallest Number: " + smallestNumber + "</li></h3>")
document.write("<h3><li>Sum: " + sum + "</li></h3>")
document.write("<h3><li>Average Value: " + avg + "</li></h3>")
document.write("<h3><li>Even Numbers: " + evens + "</li></h3>")
document.write("<h3><li>Odd Numbers: " + odds + "</li></h3>")
document.write("<h3><li>Numbers Divisible By 8: " + divisible + "</li></h3>")
document.write("</ul>");